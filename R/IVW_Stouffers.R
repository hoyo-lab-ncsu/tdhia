...
ivw_stouffers <- function(aa_output, adj_p_level=0.10, min_cpg=3, verbose=TRUE, top_n_icrs=NULL){
  ...
  recompute_z <- function(df=aa_output) {
    if (!"Statistic" %in% names(df) || any(!is.finite(df$Statistic))) {
      message("Recomputing CpG-level Z as Estimate/StdError where needed.")
      df$Statistic <- with(df,
  ...
  results <- purrr::map_dfr(icr_of_interest, function(icr) {
    ...
    if (!is.finite(Beta_IVW) || Beta_IVW <= 0 ||
        !is.finite(Z_ICR)    || Z_ICR <= 0    ||
        !is.finite(Effect_trait) || Effect_trait <= 0 ||
        p_ICR < 0) {
      return(tibble::tibble(
        ICR_id = icr,
...
test_ivw <- ivw_stouffers(aa_output = aa_output_test_no_strat01,
                          adj_p_level=0.20,
                          min_cpg=3,
                          verbose = TRUE,
                          top_n_icrs = 6)
