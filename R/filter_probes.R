...
#'
filter_probes <- function(probe_beta, discard_unmapped_probes = TRUE, max_sig_pval = 0.2, max_probe_fail_rate = 0.5, discard_failed_probes = TRUE) {
    # save(list = ls(all.names = TRUE), file = 'filter_probes_debug.RData') load(file = 'filter_probes_debug.RData')
    ...
    #----------------------------------------------------------------------------
    # (Defined in manifest file where MAPINFO == (0 or NA)) Data is copied over to filter_* probe_beta and probe_ pval matrices.
    if (discard_unmapped_probes) {
        f_unmmaped = function(x) is.na(x) | x == 0
        unmapped_probe_ids <- sapply(mft$Probe_ID, function(x) f_unmmaped(mft$MAPINFO[which(x == mft$Probe_ID)[1]]))
        ...
        cat(sprintf("Probe filter: discarding %.0f%% probes ( %i/ %i) b/c they don't
            ...
            100 * nrow(filt_probe_beta_df)/nrow(probe_beta$probe_beta_df), nrow(probe_beta$probe_beta_df) - nrow(filt_probe_beta_df), nrow(probe_beta$probe_beta_df),
    ...
    if (!is.null(max_sig_pval)) {
        ...
        cat(sprintf("Probe filter: %.0f%% of beta measurements ( %i/ %i) failed the
            ...
            100 * sum(is.na(filt_probe_beta_df))/prod(dim(filt_probe_beta_df)), sum(is.na(filt_probe_beta_df)), prod(dim(filt_probe_beta_df)),
            max_sig_pval))
    }
    # hist(rowSums(!is.na(filt_probe_beta_df))/ncol(filt_probe_beta_df), main = paste('Histogram of probe pass rate'), xlab = 'Fraction
    # passed probes')
    ...
    if (!is.null(max_probe_fail_rate)) {
        ...
        row_fail_flag <- rowSums(is.na(filt_probe_beta_df))/ncol(filt_probe_beta_df) > max_probe_fail_rate
        # Set rows to NA
        ...
        cat(sprintf("Probe filter: %.0f%% of probes ( %i/ %i) had a signal p-value
            ...
            100 * sum(row_fail_flag)/length(row_fail_flag), sum(row_fail_flag), length(row_fail_flag), 100 * max_probe_fail_rate))
    }
    # hist(rowSums(!is.na(filt_probe_beta_df))/ncol(filt_probe_beta_df), main = paste('Histogram of probe pass rate'), xlab = 'Fraction
    # passed probes')
    ...
    cat(sprintf("Probe filter: discarded %.0f%% probes ( %i/ %i) because all measurements were NA.
              %i probes now remain. \n", 100 *
        (nrow(filt_probe_beta_df) - nrow(filt_probe_beta_df2))/nrow(filt_probe_beta_df), nrow(filt_probe_beta_df) - nrow(filt_probe_beta_df2),
        nrow(filt_probe_beta_df), nrow(filt_probe_beta_df2)))

    # hist(rowSums(!is.na(filt_probe_beta_df2))/ncol(filt_probe_beta_df2), main = paste('Histogram of probe pass rate'), xlab = 'Fraction
    # passed probes')


    probe_beta <- list(probe_beta_df = filt_probe_beta_df, probe_pval_df = filt_probe_pval_df, platform = probe_beta$platform, manifest = probe_beta$manifest)
