...
#'
#'
convert_probes_to_cpgs <- function(probe_beta, quantile_norm = FALSE, discard_unmapped_cpgs = TRUE) {
    # Get manifest data
    ...
    probe_beta_df2 <- probe_beta$probe_beta_df %>%
        ...
        dplyr::left_join(y = dplyr::select(mft, c("Probe_ID", "Name")), "Probe_ID", -"Probe_ID") %>%
    ...
    cpg_beta_df <- probe_beta_df2 %>%
        dplyr::group_by(.data$CpG_ID) %>%
        dplyr::summarize(dplyr::across(dplyr::where(is.numeric), function(x) mean(x, na.rm = TRUE)), n_probes = dplyr::n())
    ...
    cat(sprintf("CpG filter: discarded %.0f%% of CpG entries ( %i/ %i) from manifest because
        ...
        100 * (n_unique_cpgs_mft - n_unique_cpgs_data)/n_unique_cpgs_mft, n_unique_cpgs_mft - n_unique_cpgs_data, n_unique_cpgs_data, n_unique_cpgs_data))



    # Discard CpG_ID(s) that do not map uniquely to a single genome location Defined in manifest file where MAPINFO == (0 or NA)
    if (discard_unmapped_cpgs) {
        f_unmmaped = function(x) is.na(x) | x == 0
        unmapped_cpg_ids <- sapply(cpg_beta_df$CpG_ID, function(x) f_unmmaped(mft$MAPINFO[which(x == mft$Name)[1]]))
      ...
      do not map uniquely to the genome. %i CpG sites remain.\n",
            100 * sum(unmapped_cpg_ids)/length(unmapped_cpg_ids), sum(unmapped_cpg_ids), length(unmapped_cpg_ids), length(unmapped_cpg_ids) -
                sum(unmapped_cpg_ids)))

    }

    cpg_beta <- list(cpg_beta_df = cpg_beta_df, platform = probe_beta$platform, manifest = probe_beta$manifest)
